    ...
    m_currentAnimState(BattleAnimationState::Idle) // アニメーション状態を初期化
{
    ...
    Deck_yama.resize(getData().Deck.size());
    for (int i = 0;i < Deck_yama.size(); i++)Deck_yama.at(i) = i;
    // Deck_yama = Deck;
    Deck_yama.shuffle();
...
void Battle::updateTableDeck()
{
    ...
    for (int i = 0; i < getData().Deck.size(); i++) {
        if (getData().Deck.at(i).GetStat() == 2 && Deck_board.includes(getData().Deck.at(i)) == false) {
            // 手札のブロックが盤面に移動している場合、盤面に追加する
            Deck_board.push_back(getData().Deck.at(i));
        }
        if (getData().Deck.at(i).GetStat() == 1 && Deck_table.includes(i) == false) {
...
void Battle::updateDiscardEffect()
{
    ...
    if (table_id < Deck_table.size()) {
        if (sutehuda_angle > -90_deg) {
            ...
            if (tehuda_rate > 0.99) {
                ...
                Deck_gomi.emplace_back(getData().Deck.at(Deck_table[table_id])); // 手札のブロックを捨て札に移動
    ...
    for (auto& block : Deck_table) {
        // 手札のブロックを捨て札に移動
        getData().Deck.at(block).SetStat(-1); // ブロックのステータスを捨て札に設定
        Deck_gomi.push_back(getData().Deck.at(block));
    }
    Deck_table.clear();
    // (敗北判定もここで行う)
...
void Battle::updateCardDrawEffect()
{
    ...
    for (int32 i = global_id; i < table_size; ++i) {
        ...
        // 山札からカードを1枚引く
        int card = Deck_yama.back();
        Deck_yama.pop_back(); // 山札から削除
        Deck_table.push_back(card); // 手札に追加
        getData().Deck.at(Deck_table.back()).SetStat(1); // 手札のステータスを1に設定
        getData().Deck.at(Deck_table.back()).SetPos(300 + i * 15, 700); //
    }
...
void Battle::update()
{
    ...
    for (int i = 0; i < Deck_table.size(); ++i) {
        if (getData().Deck.at(Deck_table[i]).IsDragging() && !board_locked) {
            // Edit here
        ...
        if (getData().Deck.at(Deck_table[i]).IsHovered() && !board_locked) {
...
void Battle::drawDiscardEffect() const
{
    ...
    auto [sx, sy] = getData().Deck.at(Deck_table[table_id]).GetPos();
    Vec2 pos = Vec2{ sx, sy }.lerp(Vec2{ 1560, 750 }, tehuda_rate); // 手札の位置を取得
    getData().Deck.at(Deck_table[table_id]).Draw({ (int32)pos.x, (int32)pos.y }, 1.0, sutehuda_angle);
...
void Battle::drawCardDrawEffect() const
{
    auto [sx, sy] = getData().Deck.at(Deck_table[table_id]).GetPos();
    Vec2 pos = Vec2{ sx, sy }.lerp(Vec2{ 1560, 750 }, tehuda_rate); // 手札の位置を取得
    getData().Deck.at(Deck_table[table_id]).Draw({ pos.x, pos.y }, 1.0, yamahuda_angle);
}
