...
inline constexpr std::int32_t MaximumDamageHitCount = 8;
inline constexpr std::int32_t DamagePercentageForMaximumHitCount = 50;
...
// 最大HPの半分で最大回数となるよう，ダメージを複数ヒットへ変換する．
[[nodiscard]] inline std::int32_t HitCountForDamage(
	...
	const std::int64_t scaled_damage = static_cast<std::int64_t>(actual_damage)
		* 100 * MaximumDamageHitCount;
	const std::int64_t scaled_maximum = static_cast<std::int64_t>(maximum_hp)
		* DamagePercentageForMaximumHitCount;
	const std::int32_t count = static_cast<std::int32_t>(
		(scaled_damage + scaled_maximum - 1) / scaled_maximum);
	return std::min({ count, MaximumDamageHitCount, actual_damage });
}

[[nodiscard]] inline constexpr double ResolveDefenseExchangeDuration(
	const std::int32_t attack, const std::int32_t defense) noexcept {
	if ((attack <= 0) || (defense <= 0)) return 0.0;
	const std::int32_t blocked_damage = std::max(0,
		std::min(std::max(0, attack), std::max(0, defense)));
	return MaximumDefenseExchangeDuration * blocked_damage / defense;
}
